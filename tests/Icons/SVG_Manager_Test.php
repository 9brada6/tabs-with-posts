...
/**
 * Tests the SVG_Manager class methods that remains untested. Other tests are
...
class SVG_Manager_Test extends WP_UnitTestCase {
	...
	public function test__get_compatible_disabled_comment_icon_id() {
		$icon_id = SVG_Manager::get_compatible_disabled_comment_icon_id( 'wrong_id' );
		$this->assertNull( $icon_id );

		// @phan-suppress-next-line PhanTypeMismatchArgument
		$icon_id = SVG_Manager::get_compatible_disabled_comment_icon_id( 30 );
		$this->assertNull( $icon_id );

		$icon = SVG_Manager::get_compatible_disabled_comment_icon( 'wrong_id' );
	 ...
	 * @covers \TWRP\Icons\SVG_Manager::get_icon_category
	 */
	public function test__get_icon_attr__get_icon_category() {
		...
		foreach ( $icons_attr as $icon_id => $icon_attr ) {
			if ( SVG_Manager::get_icon_attr( $icon_id ) !== $icon_attr ) {
				$works = false;
			}
		}
		...
		$this->assertFalse( SVG_Manager::get_icon_attr( 'bad-icon' ) );

		$this->expectException( RuntimeException::class );
		SVG_Manager::get_icon_category( 'bad-icon' ); // @phan-suppress-current-line PhanThrowTypeAbsentForCall
	}
}
