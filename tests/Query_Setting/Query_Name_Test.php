...
class Query_Name_Test extends TestCase {
	...
	public function test__get_submitted_sanitized_setting__not_set() {
		$class                   = self::$class_instance;
		$sanitized_submitted_val = $class->get_submitted_sanitized_setting();

		$this->assertTrue( $class::get_default_setting() === $sanitized_submitted_val );
	}
	 ...
	 * @param mixed $value
	 */
	public function test__get_submitted_sanitized_setting__values( $value ) {
		$class = self::$class_instance;
	...
	public function sanitization_provider() {
		...
		return array_merge( $basic_values, $setting_values );
	}
