...
class BlazK_Plugin implements Post_Rating_Plugin {
	...
	public static function is_installed_and_can_be_used() {
		if ( ! function_exists( 'rmp_get_avg_rating' ) ) {
			return false;
		}

		if ( ! function_exists( 'rmp_get_vote_count' ) ) {
			return false;
		}
	...
	public static function modify_query_arg_if_necessary( $query_args ) {
		$orderby_value = \TWRP\Query_Generator\Query_Setting\Post_Order::PLUGIN_BLAZK_ORDERBY_VALUE;
		...
		$query_args['meta_key'] = 'rmp_avg_rating'; // phpcs:ignore -- using meta_key is making query slow.
