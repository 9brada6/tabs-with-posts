...
class Simple_Article_Block extends Article_Block {
	...
	public function get_components() {
		...
		$title_css_components     = array(
			$block_class . ' .twrp-ss__title' => Widget_Component_Settings::TEXT_SETTINGS,
			$block_class . ' .twrp-ss__link:hover .twrp-ss__title' => array( Widget_Component_Settings::HOVER_COLOR_SETTING ),
		);
		...
		$date_css_components = array(
			$block_class . ' .twrp-ss__date' => Widget_Component_Settings::TEXT_SETTINGS,
			$block_class . ' .twrp-ss__link:hover + .twrp-ss__meta-wrapper .twrp-ss__date' => array( Widget_Component_Settings::HOVER_COLOR_SETTING ),
		);
		...
		$author_css_components = array(
			$block_class . ' .twrp-ss__author' => Widget_Component_Settings::TEXT_SETTINGS,
			$block_class . ' .twrp-ss__link:hover + .twrp-ss__meta-wrapper .twrp-ss__author' => array( Widget_Component_Settings::HOVER_COLOR_SETTING ),
		);
	...
	public function display_form_settings() {
		// Display Post Thumbnail Setting.
		$id              = Widget_Utils::get_field_id( $this->widget_id, $this->query_id, 'display_post_thumbnail' );
		$name            = Widget_Utils::get_field_name( $this->widget_id, $this->query_id, 'display_post_thumbnail' );
		$current_setting = ( isset( $this->settings['display_post_thumbnail'] ) && is_string( $this->settings['display_post_thumbnail'] ) ) ? $this->settings['display_post_thumbnail'] : null;
		Checkbox_Control::display_setting( $id, $name, $current_setting, $this->get_display_post_thumbnail_setting_args() ); // @phan-suppress-current-line PhanPartialTypeMismatchArgument
	...
	public function sanitize_widget_settings( $set_internal = true ) {
		...
		$post_thumbnail_setting_to_sanitize           = isset( $settings['display_post_thumbnail'] ) ? $settings['display_post_thumbnail'] : null;
		$sanitized_settings['display_post_thumbnail'] = Checkbox_Control::sanitize_setting( $post_thumbnail_setting_to_sanitize, $this->get_display_post_thumbnail_setting_args() );
		...
		$date_setting_to_sanitize           = isset( $settings['display_date'] ) ? $settings['display_date'] : null;
		$sanitized_settings['display_date'] = Checkbox_Control::sanitize_setting( $date_setting_to_sanitize, $this->get_display_date_setting_args() );
