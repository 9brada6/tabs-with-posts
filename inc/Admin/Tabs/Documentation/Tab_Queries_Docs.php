...
class Tab_Queries_Docs {
	...
	public function display() {
		?>
		<h2 class="<?php $this->bem_class( 'title-section' ); ?>"><?= esc_html_x( 'Tab Queries', 'backend, documentation', 'tabs-with-posts' ); ?></h2>
		...
		<p>
			<?=  esc_html_x( 'There are a lot of options of which you can create your own query, you can see how these work by applying a starter template, and modify from there, maybe add more post types, or exclude some categories... the number of possibilities is big.', 'backend, documentation', 'tabs-with-posts' ); ?>
		</p>
		...
		<p>
			<?=  esc_html_x( 'Below are some of the setting that might be confused, explained:', 'backend, documentation', 'tabs-with-posts' ); ?>
		</p>

		<h3 class="<?php $this->bem_class( 'title-sub-section' ); ?>"><?=  esc_html_x( 'Starting template setting', 'backend, documentation', 'tabs-with-posts' ); ?></h3>

		<p>
			<?=  esc_html_x( 'The starting templates are there to give users an insight of how the settings should work. By filling the most important ones, the user can then expand for them, changing other settings if needed.', 'backend, documentation', 'tabs-with-posts' ); ?>
		</p>

		<h3 class="<?php $this->bem_class( 'title-sub-section' ); ?>"><?=  esc_html_x( 'Name setting', 'backend, documentation', 'tabs-with-posts' ); ?></h3>

		<p>
			<?=  esc_html_x( 'Differentiate between tab queries, giving them a name. As written in the note below it, the name is for you to know what posts the tab is displaying, and is not displayed in the frontend.', 'backend, documentation', 'tabs-with-posts' ); ?>
		</p>

		<h3 class="<?php $this->bem_class( 'title-sub-section' ); ?>"><?=  esc_html_x( 'Post statuses setting', 'backend, documentation', 'tabs-with-posts' ); ?></h3>
		...
		<p>
			<?=  esc_html_x( 'By writing the custom post statuses in the advanced query settings, you somehow realize that some unwanted posts might show to the users, and you become more carefully.', 'backend, documentation', 'tabs-with-posts' ); ?>
		</p>

		<h3 id="<?php $this->bem_class( 'ordering-settings' ); ?>" class="<?php $this->bem_class( 'title-sub-section' ); ?>"><?=  esc_html_x( 'Order of posts setting', 'backend, documentation', 'tabs-with-posts' ); ?></h3>

		<p>
			<?=  esc_html_x( 'There are 3 order options, you can use all 3 or none. Usually you must set something to be sure that what is displayed is what you want. The most common ordering is by date in descending order(to get last posts).', 'backend, documentation', 'tabs-with-posts' ); ?>
		</p>

		<p>
			<?=  esc_html_x( 'Because most of the ordering itself is pretty much self-documented by the name, below are just some of the settings that might have some quirks.', 'backend, documentation', 'tabs-with-posts' ); ?>
		</p>

		<p>
			<b><?=  esc_html_x( 'Ordering by searching terms:', 'backend, documentation', 'tabs-with-posts' ); ?></b>
			<?=  esc_html_x( 'Order by search terms in the following order: First, whether the entire sentence is matched. Second, if all the search terms are within the titles. Third, if any of the search terms appear in the titles. And, fourth, if the full sentence appears in the contents.', 'backend, documentation', 'tabs-with-posts' ); ?>
		</p>

		<p>
			<b><?=  esc_html_x( 'Meta order:', 'backend, documentation', 'tabs-with-posts' ); ?></b>
		...
		<p>
			<b><?=  esc_html_x( 'Views order:', 'backend, documentation', 'tabs-with-posts' ); ?></b>
		...
		<h3 class="<?php $this->bem_class( 'title-sub-section' ); ?>"><?=  esc_html_x( 'Meta key setting', 'backend, documentation', 'tabs-with-posts' ); ?></h3>

		<p class="<?php $this->bem_class( 'developer-text' ); ?>">
			<?=  esc_html_x( 'By changing this setting, you must know what is the meta key that you should modify, the meta value(shown if you have a comparator) is verified if it is a number, and either compared as a number, or as a string(and is added to either meta_value, or meta_value_num key).', 'backend, documentation', 'tabs-with-posts' ); ?>
		</p>
