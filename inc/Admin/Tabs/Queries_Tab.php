...
class Queries_Tab implements Interface_Admin_Menu_Tab
{
    ...
    {
        $existing_queries = DBQueryOptions::get_all_queries();
        ...
        // todo: delete.
        var_dump(\TWRP\QueryPosts::get_wp_query_arguments(6));
    ...
    protected function get_query_edit_link( $query_id )
    {
        ...
        if (DBQueryOptions::query_exists($query_id)) {
    ...
    {
        $setting_classes = ManageComponentClasses::get_registered_backend_settings();
        ...
        <div class="twrp-posts-queries-tab">
            <form action="<?= esc_url($this->get_edit_query_form_action()); ?>" method="post">
                <?php foreach ($setting_classes as $setting_class) : ?>
                    <?php
                    $current_setting = DBQueryOptions::get_specific_query_setting($query_id, $setting_class->get_setting_name());
    ...
    protected function get_id_of_query_being_modified()
    {
		if ( isset( $_GET[ self::EDIT_QUERY__URL_PARAM_KEY ] ) ) { // phpcs:ignore -- Nonce verified.
            ...
            if (is_numeric($key) && DBQueryOptions::query_exists($key)) {
                return $key;
            }
        }
    ...
    protected function update_form_submitted_settings()
    {
        ...
        if ('' === $query_key) {
            DBQueryOptions::add_new_query($query_settings);
        } elseif (DBQueryOptions::query_exists($query_key)) {
            DBQueryOptions::update_query($query_key, $query_settings);
        }
    }
    ...
    protected function execute_delete_query_action()
    {
		if ( isset( $_GET[ self::DELETE_QUERY__URL_PARAM_KEY ] ) ) { // phpcs:ignore
            ...
            if (DBQueryOptions::query_exists($key)) {
                DBQueryOptions::delete_query($key);
            }
        }
    }
}
